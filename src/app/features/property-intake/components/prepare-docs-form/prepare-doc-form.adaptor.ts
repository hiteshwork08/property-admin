...
@Injectable()
export class PrepareDocsFormAdaptor extends AbstractRequestFormAdaptor<
  PrepareDocsFormData,
  PrepareDocsRes
> {
  ...
  override onRequest(formGroup: PrepareDocsFormData) {
    this.propertyIntakeModel.propertyIntakeStatus =
      PropertIntakeFormEnum.ReceivedDocs;
  ...
  override onSuccess(
    formData: PrepareDocsFormData,
    res: PrepareDocsRes,
    formGroup: FormGroup<WrapFormControl<PrepareDocsFormData>>
  ): void {
