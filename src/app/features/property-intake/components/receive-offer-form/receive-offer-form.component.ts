...
import { FetchModule, provideFetchAdaptor } from "@common/fetch/fetch.directive";
import { FetchStatesAdaptor, INTAKE_DROPDOWN, StatesInfo } from "@common/adaptors/fetch-state.adaptor";
...
@Component({
  ...
  providers: [provideFetchAdaptor(FetchStatesAdaptor, true), provideFormAdaptor(SubmitIntakeReceiveOfferFormAdaptor, true)],
})
export class ReceiveOfferFormComponent {
  form = new FormGroup({
    ...
    APN: new FormControl<string>(null, { validators: Validators.required, nonNullable: true }),
    intakeType: new FormControl<number>(null, { validators: Validators.required, nonNullable: true }),
    sellerList: new FormControl<string[]>([]),
    dateReceived: new FormControl<Date>(new Date(), { validators: Validators.required, nonNullable: true }),
  ...
  constructor() {
    this.form.controls.selectedState.valueChanges.pipe(tap((data) => this.mapCountyBasedOnState(data))).subscribe();
  }
